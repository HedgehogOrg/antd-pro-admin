...
function SystemLogs() {
  const intlSystemLogs = useIntl('systemLogs');
  ...
  const columns: ProColumns<SystemLogType>[] = [
    ...
    {
      title: intlSystemLogs('ACTION_TIME'),
      dataIndex: 'createdAt',
      valueType: 'dateTime',
      renderFormItem: () => (
        <RangePicker
          showTime
          placeholder={[intlSystemLogs('START_TIME'), intlSystemLogs('END_TIME')]}
    ...
    {
      title: intlSystemLogs('ACTION_DETAIL'),
  ...
  return (
    <CustomProTable<SystemLogType>
      headerTitle={intlSystemLogs('SYSTEM_LOGS')}
      columns={columns}
      pagination={{
